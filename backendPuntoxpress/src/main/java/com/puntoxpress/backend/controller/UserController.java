...
public class UserController {
    ...
    @PostMapping("/crear")
    public ResponseEntity<ResponseDTO<?>> crear(@Valid @RequestBody User dataRequest) {
        ...
        try {
            User save = userService.crear(dataRequest);
    ...
    public ResponseEntity<ResponseDTO<?>> actualizar(@PathVariable UUID id, @Valid @RequestBody User dataRequest) {
        log.info("Update initiated :{}", dataRequest.toString());
        ResponseEntity<ResponseDTO<?>> responseEntity = new ResponseEntity<>(HttpStatus.OK);
        ResponseDTO<User> response = new ResponseDTO<>();
        HttpStatus httpStatus;
        try {
            User update = userService.actualizar(id, dataRequest);
            if (update == null) {
    ...
    public ResponseEntity<ResponseDTO<?>> listar(@RequestParam(defaultValue = "0") int page,
        ...
        try {
            ...
            Page<User> registrosPage = userService.obtenerLista(pageableRequest);
    ...
    public ResponseEntity<ResponseDTO<?>> buscarPorId(@PathVariable UUID id) {
        ...
        HttpStatus httpStatus;
        try {
            User registro = userService.buscarPorId(id);
            if (registro == null) {
                httpStatus = HttpStatus.NOT_FOUND;
                response.setCode(httpStatus.value());
                response.setMessage("Registro no encontrado");
