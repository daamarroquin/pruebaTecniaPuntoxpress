...
public class LocationController {
    ...
    @PostMapping("/crear")
    public ResponseEntity<ResponseDTO<?>> crear(@Valid @RequestBody Location dataRequest) {
        log.info("Create initiated :{}", dataRequest.toString());
        ResponseEntity<ResponseDTO<?>> responseEntity = new ResponseEntity<>(HttpStatus.OK);
        ResponseDTO<Location> response = new ResponseDTO<>();
        HttpStatus httpStatus;
        try {
            Location save = locationService.crear(dataRequest);
    ...
    public ResponseEntity<ResponseDTO<?>> actualizar(@PathVariable UUID id, @Valid @RequestBody Location dataRequest) {
        log.info("Update initiated :{}", dataRequest.toString());
        ResponseEntity<ResponseDTO<?>> responseEntity = new ResponseEntity<>(HttpStatus.OK);
        ResponseDTO<Location> response = new ResponseDTO<>();
        HttpStatus httpStatus;
        try {
            Location update = locationService.actualizar(id, dataRequest);
            if (update == null) {
    ...
    public ResponseEntity<ResponseDTO<?>> eliminar(@PathVariable UUID id) {
        ...
        HttpStatus httpStatus;
        try {
            boolean isDeleted = locationService.eliminar(id);
            if (isDeleted) {
                httpStatus = HttpStatus.OK;
                response.setCode(httpStatus.value());
    ...
    public ResponseEntity<ResponseDTO<?>> listar(@RequestParam(defaultValue = "0") int page,
        ...
        ResponseEntity<ResponseDTO<?>> responseEntity = new ResponseEntity<>(HttpStatus.OK);
        ResponseDTO<List<Location>> response = new ResponseDTO<>();
        HttpStatus httpStatus;
        try {
            ...
            Page<Location> registrosPage = locationService.obtenerLista(pageableRequest);
    ...
    public ResponseEntity<ResponseDTO<?>> buscarPorId(@PathVariable UUID id) {
        ...
        HttpStatus httpStatus;
        try {
            Location registro = locationService.buscarPorId(id);
            if (registro == null) {
                httpStatus = HttpStatus.NOT_FOUND;
                response.setCode(httpStatus.value());
                response.setMessage("Registro no encontrado");
